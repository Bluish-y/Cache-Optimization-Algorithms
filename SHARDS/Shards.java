...
public class Shards {
    ...
    static int P=4096*4, s = 129;
    static int T = (int)(R*P);
    
    Set<Integer> cache;
    int capacity;
    int miss=0,hit=0;
    int[] stackDist;
    PriorityQueue<Tuple> newCache;
 
    public Shards(int capacity)
    {
        this.cache = new LinkedHashSet<Integer>(capacity);
        this.newCache = new PriorityQueue<Tuple>(s, new TupleComparator());
    ...
    static HashMap<String, Integer> createHashMap(LinkedList<String> arr)
    {
        ...
        for (String arrElement : arr) {
            ...
            // Insert the element
            if (hmap.get(arrElement) == null && c%P<T) {
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        /*Created a HashMap for the traces with threshold*/
        HashMap<String,Integer> traceMap = createHashMap(traces);

        /*removing unsampled traces*/
        it = traces.listIterator();
        while(it.hasNext()){
            if(traceMap.get(it.next())==null)
                it.remove();
        }


        /* To find the stack distance call implement function*/
        int[] stackDist = implement(traces, traceMap, 129);


        for(int i = 0; i < stackDist.length; i++){
            stackDist[i] = (int)(stackDist[i]/R);
            System.out.print(stackDist[i] + " ");
        }


        /*To calculate the cost function sum all stack distances before */
